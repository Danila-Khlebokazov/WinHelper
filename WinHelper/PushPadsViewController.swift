...
class PushPadsViewController: UIViewController {
    ...
    @IBAction func padClicked(_ sender: UIButton) {
        ...
        if let index = pads.firstIndex(of:tagButton){
...
//            pressedButons.append(tagButton)
    ...
    @objc func startNewGame(){
        ...
        for _ in 0...randomNumber1{
            ...
            if let index = pads.firstIndex(of:randomNumber2){
                continue
            } else {
    ...
    func startConfetti(){
        ...
        let cells : [CAEmitterCell] = confettiColors.compactMap{
    ...
    @objc func stopConfetti(){
        if let lastSublayer = view.layer.sublayers?.last {
